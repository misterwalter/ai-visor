...
def fetch_issue(number, repo=None, cwd=None):
    ...
    fields = "number,title,body,author,labels,state,createdAt,url,comments"
...
def fetch_pr(number, repo=None, cwd=None):
    fields = ("number,title,body,author,labels,state,createdAt,url,comments,"
              "files,additions,deletions,baseRefName,headRefName")
...
def list_open(label=None, repo=None, cwd=None, limit=20):
    ...
    if label:
        args += ["--label", label]
    return json.loads(_gh(args, repo, cwd))
